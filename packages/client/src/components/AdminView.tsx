...
export default function AdminView({ onExit }: { onExit: () => void }) {
    ...
    const [activeTab, setActiveTab] = useState<
        'OVERVIEW' | 'THOUGHTS' | 'SWAPS' | 'SESSIONS' | 'LOGS'
    >('OVERVIEW');
            ...
            <header className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
                ...
                <div className="flex items-center space-x-4">
                    <div className="hidden md:grid grid-cols-2 gap-6 text-xs font-mono text-slate-500">
                        ...
                        <span>
                            CONSENTED: {data.stats.totalConsented}/{data.stats.totalUsers}
                        </span>
                                        ...
                                        <div className="w-full bg-slate-800 rounded-full h-2">
                                            <div
                                                ...
                                                style={{
                                                    width:
                                                        data.stats.totalUsers > 0
                                                            ? `${(data.stats.totalConsented / data.stats.totalUsers) * 100}%`
                                                            : '0%',
                                                }}
                                            ></div>
                                        </div>
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-4">
                                            <span className="text-2xl font-mono text-white font-bold">
                                                {session.course?.joinCode || 'UNKNOWN'}
                                            </span>
                                            <span className="px-2 py-1 bg-emerald-900/30 text-emerald-400 text-[10px] font-bold uppercase rounded border border-emerald-900/50">
                                                Active
                                            </span>
                                        </div>
                                    ...
                                    <tbody className="divide-y divide-slate-800">
                                        {data.logs.map((log: Log) => (
                                            ...
                                            >
                                                ...
                                                <td
                                                    className="px-4 py-2 text-slate-400 truncate max-w-xs"
                                                    title={JSON.stringify(log.payload)}
                                                >
                                                    {log.payload
                                                        ? JSON.stringify(log.payload).substring(
                                                              0,
                                                              50
                                                          )
                                                        : '-'}
